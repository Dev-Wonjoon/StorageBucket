...
export const FavoriteService = {
    ...
    async toggle(mediaId: number): Promise<boolean> {
        const existing = db.select().from(favorites).where(eq(favorites.mediaId, mediaId)).get()
    ...
    async isFavorite(mediaId: number): Promise<boolean> {
        const row = db.select().from(favorites).where(eq(favorites.mediaId, mediaId)).get()
        return !!row
    }
}
